...
An XOR-constraint (or parity constraint) is a boolean function $f$ in the form of $x_1 \oplus \dots \oplus x_n = \ve$ that $x_1, \dots, x_n \in \Lit$ and $\ve \in \set{0,1}$. Two XOR-constraints $f_1, f_2$ are equivalent iff $\var(f_1) = \var(f_2)$ and both have the same parity for literals.
\end{defi}
Remarks:
\begin{enumerate}
...
An XOR-clause-set is a clause-sets $F \in \Cls$ whose clauses $C$ are interpreted as an XOR-clauses. An XOR-clause $C \in \Cl$ is an XOR-constraint with $\oplus_{x \in C} = 0$. Thus, two XOR-clauses $C, D$ are equivalent iff $\var(C) = \var(D)$ and both with the same parity. 

In an XOR-clause, there is no repetitive literals since $x \oplus x = 0$. In the case of clashing literals, we remove them using $x \oplus \ol{x} = 1$ and $1 \oplus y = \ol{y}$ (since according the definition, clashing literals are not allowed in clauses).
\end{defi} 
...
The satisfiability of a CNF-clause-set is investigated using the resolution operation. Here, for investigating whether an XOR-clause-set is satisfiable or not, an operation is defined which is called XOR-sum over clauses of $F$ and denoted by $\oplus F \in \Cl$. This operation is applied to two clauses and produces a new clause or the result is inconsistent. The operation proceeds as follows. If there are four literals as $x,\ol{x},y,\ol{y}$ for $x \ne y$, the XOR-sum of them is as $x \oplus \ol{x} = 1$ and $y \oplus \ol{y} = 1$. Thus, $1 \oplus 1 = 0$ and they are removed. If no clash remains, then the result is obtained as a clause $E \in \Cl$. Else if just two literals $x, \ol{x}$ remains with at least one other literal $y$, then $x \oplus \ol{x} = 1$ and $1 \oplus y=\ol{y}$ and a new clause $E \in \Cl$ is obtained. Otherwise, if there is just two literals $x, \ol{x}$, then $\oplus F$ is called inconsistent. An XOR-clause-set $F \in \Cls$ is unsatisfiable if and only if there is $F' \sse F$ such that $\oplus F'$ is inconsistent.
...
For each XOR-clause-set $F$ there is a unique equivalent CNF-clause-set $F'$. To obtain this equivalent CNF-clause-set, a translation is defined and indicated by $X_0(F)$. This translation, converts each clause of $F$ to a set of full clause-sets in CNF-representation and then $F'$ is the union of these clause-sets. If $F$ has long clauses, then obtaining $X_0(F)$ is not practical.

\begin{defi}\label{def:x0tr}
The $X_0$ translation is a map $X_0: \Cls \ra \Cls$ where the input is an XOR-clause-set and the output is a  CNF-clause-set. Consider an XOR-clause $C \in F$, the $X_0$ translation of this clause is defined as the set of prime implicates of the boolean function for $C$ and indicated by $X_0(C)=\primec_0(x_1 \oplus \dots \oplus x_n = 0)$. The translation $X_0(C)$ contains all the full clauses over the set of variables in $C$ whose parity are different from the parity of $C$. Thus, $\abs{X_0(C)}=2^{\var(C)-1}$. Finally, the CNF-clause-set is obtained by $X_0(F) := \bc_{C \in F} X_0(C)$.
\end{defi} 
...
A graph is a pair $G = (V,E)$ where $V$ is the set of vertices and $E$ is the set of edges. If we consider the graph as a triple $G = (V,E,\eta)$ such that $V$ is the set of vertices, $E$ is the set of edge-labels and $\eta: E \ra \set{e \sse V : 1 \le \abs{e} \le 2}$, we can have parallel edges and loops. Now, if we assign a charge to each vertex indicated by $\rho: V \ra \set{0,1}$, we obtain a unique XOR-clause-set correspond to a graph $G$. In this case, each vertex $w \in V(G)$ yields an XOR-clause $C \in \Cl$ and is defined as $\oplus_{x \in E(G), w \in \eta(x)} \; x = \rho(w)$. Then, the XOR-clause-set for $G$ is obtained by $T_0(G,\rho) := \set{C_w : w \in V(G)} \in \Cls$. The graph $G$ is called Tseitin graph. $T_0(G,\rho)$ is unsatisfiable if $G$ has no loops and $\oplus_{w \in V(G)} \rho(w) = 1$.The reason is that $\oplus_{w \in V(G)} \oplus_{x \in E(G) and w \in \eta(x)} x = 0$. Finally the equivalent CNF-clause-set of $G$ is obtained by $X_0$ translation which is called Tseitin clause-set of $G$ and indicated by $T(G,\rho) := X_0(T_0(G,\rho))$.
