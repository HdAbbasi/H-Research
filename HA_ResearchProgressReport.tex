...
\begin{defi}\label{def:Decompositions}
      A \textbf{partition} of a set $X$ is some $P \sse \pot(X) \sm \set{\es}$ which is disjoint, i.e., $\fa\, A, B \in P : A \ne B \Ra A \cap B = \es$, 
	  and where $\bc P = X$.
\end{defi}
...
\begin{examp}\label{exp:hd2}
	 ...
	 \begin{figure}[h]
      ...
      \end{tabular}
      \caption{The atomic moves for figure \ref{fig:hd2} and  $\hardness(K_4)=4$.}
      \label{fig:game2}
      \end{figure}
	  %---
	  \begin{figure}
...
\begin{examp}\label{exp:hd3}
	   ...
	   ($K_4$ graph) and the rest of the game would be the same as exqample \ref{exp:hd2} and $\hardness(K_5)=7$.
      ...
      \hline
				  ...
				  nothing & choosing 2  \\ \hline
      \end{tabular}
      \caption{The atomic moves for figure \ref{fig:hd3} and \hardness(K_5)=7.}
      \label{fig:game3}
      \end{figure}
	   %-----------------
	  \begin{figure}
...
\begin{examp}\label{exp:hd4}
      ...
      \end{tabular}
      \caption{The atomic moves for figure \ref{fig:hd4}.}
      \label{fig:game4}
      \end{figure}
	   %-----------------
	  \begin{figure}
...
\begin{examp}\label{exp:hd6}
       Consider figure \ref{fig:hd6}, a graph with two components 1 and 2 with $\hardness=5$ which have biggest hardness compared
...
According to \cite{h17} for a given a SAT instance, two graphs can be constructed, following two models. First is the Variable Incidence Graph model (VIG), that 
vertices represent variables, and edges represent the existence of a clause relating two variables. A clause for $n$ variables results 
into $ n \choose 2$ edges, one for every pair of variables. Second is the Clause-Variable Incidence Graph model (CVIG) that vertices 
represent either variables or clauses, and edges represent the occurrence of a variable in a clause. In both models modularity can be 
measured using their proposed formula.

It is obvious that the maximal modularity would be obtained putting all vertices in the same community. To avoid this problem, modularity is defined 
as the fraction of edges connecting vertices of the same community minus the expected fraction of edges for a random graph with the same 
number of vertices and same degree \cite{h17}.
%--------------------------------------------------------------------------------------------------------------------------------------
\newpage
