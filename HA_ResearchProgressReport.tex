...
\begin{defi}\label{def:models}
  For $F, G \in \Cls$ the \textbf{implication-relation} is defined as: \bmm{F \models G} if $\fa\, \vp \in \Pass : \vp * F = \top \Ra \vp * G = \top$. For $C \in \Cl$ we write \bmm{F \models C} if $F \models \set{C}$.
\end{defi}
Remarks:
\begin{enumerate}
...
\item For $F \in \Pcls{2}$ the closure of $F$ under resolution is in $\Pcls{2}$.
\end{enumerate}

\begin{examp}\label{exp:resolution1}
       For $C,D,X,Y \in \Cl$, let $C=\{a,b, \neg c\}$ and $D=\{\neg a, \neg d\}$. The resolvent of $C,D$ is $\{b, \neg c, \neg d\}$.
	   But for $X=\{a,b, \neg c\}$ and $Y=\{\neg a, \neg b, d\}$, they are nor resolvable since $\mid X \cap \overline Y \mid \not = 1$.
\end{examp}
%--------------------------------------------------------------------------------------------------------------------------------------
\section{Trees}
\label{sec:Trees}
...
\begin{defi}\label{def:hardness1}
      For $F \in \Usat$ let \textbf{hardness} $\hardness(F) \in \NNZ$ be the minimal $k \in \NNZ$ such that a resolution tree $T : F \vdash \bot$ 
...
\begin{defi}\label{def:whd1}
     For  $F \in \Usat$ the \textbf{symmetric width wid(F)} $\in \NNZ$ is the smallest $k \in \NNZ$ such that there is
	 $T : F \vdash \bot$ with $\widehat{F}(T) \in k-\Cls$.
\end{defi}
\begin{defi}\label{def:whd2}
      For a resolution tree $T$ its \textbf{(asymmetric) width whd(T)}$\in \NNZ$ is defined as $0$ if $T$ is trivial (i.e., $\mid nds(T) \mid = 1$), 
	  while otherwise for left and right children $w_1,w_2$ with subtrees $T_1, T_2$ we define
	  $$ whd(T) := max (whd(T_1),whd(T_2), min(\mid C(w_1)\mid, \mid C(w_2)\mid))$$
	  We write $R : F \vdash^k C$ if $R : F \vdash C$ and $whd(R) \leq k$. Now for $F \in \Usat$ we define $whd(F) := min\{whd(T ) \mid T : F \vdash \bot \} \in \NNZ$ \cite{h5}. 
	  %For $k \in \NNZ$ let $\Wc_k := \{F \in \Cls : whd(F) \leq k\}$.
\end{defi}
Remarks:
	    \begin{enumerate}
              \item For all $F \in \Cls $ holds whd(F)$ \leq \hardness(F)$.
		\end{enumerate}
\begin{examp}\label{exp:whd1}
      Some examples for $wid(F)$ and $whd(F)$ are as below:
	    \begin{enumerate}
              \item $wid(\{\bot \}) = whd(\{\bot \}) = 0$.
			  \item More generally for $C \in \Cl$ holds $wid(\{C\}) = whd(\{C\}) = 0$.
			  \item In general we have $wid(F) = 0 \Leftrightarrow whd(F) = 0$ for all $F \in \Cls$.
			  \item For a Horn clause-set $F$ holds $whd(F) \leq 1$ (since unit-clause propagation is sufficient to derive unsatisfiability), 
...
\begin{examp}\label{exp:whd2}
	  ...
	  Using definition \ref{def:whd2}, we have $whd(T_1) = 1$ and $whd(T_2) = 2$ and finally $whd(F) = 1$. $wid(F) = 2$ is also 
...
\begin{defi}\label{def:ex-whd}  
      The \textbf{width-hardness} $whd : \Cls \rightarrow \NNZ$  is defined for $F \in \Cls$ as follows:
	  \begin{enumerate}
              \item If $F \in \Usat$ , then $whd(F)$ is the minimum $k \in \NNZ$ such that k-resolution refutes $F$, that is, such that
			  $T : F \vdash \bot$ exists where for each resolution step $R = C  \diamond D$ in $T$ we have $ \mid C \mid \leq k$ or $\mid D \mid \leq k$.
			  \item If $F = \top$, then $whd(F) := 0$.
			  \item If $F \in \Sat \ \{ \top \}$, then $whd(F) := max \{whd(\varphi * F) : \varphi * F \in \Usat\}$.
      \end{enumerate}
\end{defi}
...
A full Tseitin graph is a dual system of XOR-constraints $(G, ρ)$, where $G$ is a connected irreflexive general graph with 
$\oplus_{w \in V(G)} \rho(w) = 1$, where irreflexive general graphs says $\forall v \in E(G) : \mid e(G)(v)\mid = 2$. Note 
that additionally to ordinary (full) Tseitin graphs we allow parallel edges, but still loops are disallowed (a loop at a
vertex in effect deactivates the corresponding equation).
The variables $v \in var(F)$ are the edges of $G$:
\begin{enumerate}
            \item If $G′ := G − v$ is still connected, then $hv → 0i ∗ F$ and $hv → 1i ∗ F$ are both isomorphic to
			$X_0(G')$. Note that $G′$ is still a Tseitin graph.
			\item Otherwise let $G',G''$ be the connected components of $G$ (both again Tseitin graphs). Now $hv → 0i ∗ F$ 
			and $hv → 1i ∗ F$ are isomorphic, in some order, to $X0(G'),X0(G'')$.
			\end{enumerate}
The endpoint of splitting is reached when $G$ is the one-point graph (which can not have edges, since loops are not allowed).
%--------------------------------------------------------------------------------------------------------------------------------------
...
\begin{lem}\label{lem:game1}
      Let $G$ be a Tseitin graph. Then $hd(X_0(G))$ is characterised by the following game:
	  \begin{enumerate}
...
\begin{examp}\label{exp:hd1}
       Fig \ref{fig:game1} showes the atomic moves of hardness game for garph $G1$ in Fig \ref{fig:hd1}. Delayer starts the game and two 
	   players play in turn. Finally, $hd(G1)=2$.
       ...
       \begin{tabular}{|c|c|} 
                  ...
                  choosing 1 & choosing 3  \\ \hline
                  nothing & choosing 2  \\ \hline
       \end{tabular}
       \caption{The atomic moves for \ref{fig:game1} and hd(F)=2.}
       \label{fig:game1}
      \end{figure}
	   %----------------------
	  \begin{figure}
...
\begin{examp}\label{exp:hd2}
       Fig \ref{fig:game2} showes the atomic moves of hardness game for garph $G1$ in Fig \ref{fig:hd2}. Delayer starts the game and two 
	   players play in turn. Finally, $hd(G1)=4$.
	  \begin{figure}[h]
      \centering
      \begin{tabular}{|c|c|} 
      \hline
                  Delayer & Prover \\ \hline
                  choosing 2 & choosing 1  \\ \hline
                  choosing 6 & choosing 4  \\ \hline
...
\begin{examp}\label{exp:hd3}
       Fig \ref{fig:game3} showes the atomic moves of hardness game for garph $G1$ in Fig \ref{fig:hd3}. Delayer starts the game and two 
	   players play in turn. Finally, $hd(G1)=3$.
	   \begin{figure}[h]
      \centering
      \begin{tabular}{|c|c|} 
      \hline
                  Delayer & Prover \\ \hline
                  choosing 9 & choosing 2  \\ \hline
                  choosing 5 & choosing 1  \\ \hline
                  choosing 7 & choosing 6  \\ \hline
      \end{tabular}
      \caption{The atomic moves for \ref{fig:game2} and hd(F)=4.}
      \label{fig:game3}
      \end{figure}
...
\begin{lem}\label{lem:game1}
      Let $G$ be a Tseitin graph. Then $whd(X_0(G))$ is characterised by the following game:
	  \begin{enumerate}
	  \item An atomic move for the current non-trivial Tseitin graph $G$ replaces $G$ with a sub-graph $G'$ of $G$, obtained by choosing 
	  some $e \in E(G)$ and choosing a connected component of $G - e$.
	  \item The two players play in turns, and delayer starts with $G$.
	  \item A move of delayer is to apply a sequence of atomic moves (possibly zero).
	  \item A mover of prover is to replace the current global sequence of atomic moves by another sequence, which is consistent with 
	  the old sequence, and handles exactly one new edge.
	  \item Here "consistent" means that in case removal of an edge splits the graph into two connected components, where this edge 
	  occurs also in the original sequence, then the same “side” of the graph is chosen.
	  \item The games ends when $G$ becomes trivial, in which case delayer gets as many points as the maximum length of a sequence 
	  used in a replacement by prover.
	  \end{enumerate}
\end{lem}
